...
Page(Object.assign({}, baseHandle, {
  ...
  onSelfAddress: function () {
    ...
    if (that.data.goodsDetailInfo != null) {
      addressUrl += "?goodsId=" + that.data.goodsDetailInfo.id + "&buyType=" + that.data.buyType + "&relationId=" + that.data.orderInfo.relationId + "&type=act&activityId=" + that.data.orderInfo.activityId;
    }
    wx.redirectTo({
      url: addressUrl,
    })
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if (data && 0 == data.ret) {
          ...
          /* 配送方式判断  */
          if(goodsDetailInfo.isVirtual==1){ //虚拟商品
            that.setData({
              deliveryTab:false
            });
          }
          else if (goodsDetailInfo.wayType==="0") { //两种方式
            that.setData({
              deliveryTab:true,
              ecWayType:1
            });
          }else if (goodsDetailInfo.wayType==="1" || goodsDetailInfo.wayType==="3") { //只有一种配送方式
            that.setData({
              deliveryTab: false,
              ecWayType:goodsDetailInfo.wayType
            })
          } else if (!goodsDetailInfo.wayType && that.data.getSelf) { //以前旧数据兼容
            that.setData({
              deliveryTab: true
            });
          } else if (!that.data.getSelf) { //暂未开启自提
            that.setData({
              deliveryTab: false
            });
          }
  ...
  commitOrder: function (e) {
    ...
    console.log(buyType)
    wx.request({
      url: cf.config.pageDomain + '/mobile/base/activity/busi/confirmOrder',
      data: {
        cusmallToken: cusmallToken,
            ...
            'success': function (res) {
              ...
              wx.showModal({
                ...
                success: function (res) {
                  if (res.confirm) {
                    // wx.redirectTo({
                    //   url: "/pages/groupbuy/groupbuy?activityId=" + that.data.orderInfo.activityId + "&orderNo=" + that.data.orderRetData.orderNo
                    // })
                    wx.redirectTo({
