...
Page(Object.assign({}, baseHandle, {
  ...
  onLoad: function (options) {
    ...
    app.getUserInfo(this,options,function (userInfo, res) {
      ...
      that.reqAtyingGoodsDetail(options.activityId, options.goodsId, options.relationId);
  ...
  //选择自提点 activityId=8AB206C5ED2F4DDA985BCE2969EFEB60& goodsId=926& buyType=1
  onSelfAddress: function () {
    var that = this;
    var addressUrl = '/pages/selfAddress/selfAddress';
    if (that.data.goodsDetailInfo != null) {
      addressUrl += "?goodsId=" + that.data.goodsDetailInfo.id + "&relationId=" + that.data.relationId+ "&buyType=" + that.data.buyType + "&type=groupbuy&activityId=" + that.data.orderInfo.activityId + "&tuanLeader=" + that.data.tuanLeader;
    }
    wx.redirectTo({
      url: addressUrl,
    })
  ...
  reqAtyingGoodsDetail: function (activityId, awardsTypeId, relationId) {
    wx.showLoading({
      title: '加载中',
    });
    let that = this;
    wx.request({
      ...
      data: {
        ...
        type: relationId ? 1 : 0,//有关系则优惠购买 没关系则原价购买
      ...
      success: function (res) {
        ...
        if (data && 0 == data.ret) {
          ...
          if (relationId){
            data.model.theRelationshipDefine.createTime = util.formatTime(new Date(data.model.theRelationshipDefine.createTime));
          }
          ...
          if (data.model.activity.activityType == 13) {
            that.setData({
              isPreSale: true
            })
          }
  ...
  commitOrder: function (e) {
    ...
    isLoading = true;
    let submitUrl = cf.config.pageDomain + '/mobile/base/activity/busi/confirmOrder';
    if(that.data.isNewGb){
      submitUrl = cf.config.pageDomain + '/mobile/base/activity/busi/confirmGrouponOrder';
    }
    if(that.data.isStepGb){
      submitUrl = cf.config.pageDomain + '/mobile/base/activity/busi/confirmStepGroupon';
    }
    if(that.data.isPreSale){
      submitUrl = cf.config.pageDomain + '/mobile/base/activity/busi/confirmPreSale';
    }
    wx.request({
      url: submitUrl,
      data: {
      ...
      success: function (res) {
        ...
        if (data && 0 == data.ret) {
          ...
          let endTime = new Date(new Date().getTime() + (data.model.activityPayOrder.effectTime - data.model.activityPayOrder.orderTime));
          atyTimer = setInterval(function () { that.showCountDown(endTime, that); }, 1000);
        } else {
          wx.showModal({
            title: '提示',
  ...
  payForOrder: function(){
    ...
    if (0 != that.data.orderRetData.price){
      let orderInfo = that.data.orderInfo;
      // 新版团购支付前先校验
      if (orderInfo.relationId && (that.data.isNewGb || that.data.isStepGb)){
        wx.request({
          url: cf.config.pageDomain + "/mobile/base/activity/busi/checkRelationChain",
          data: {
            cusmallToken: cusmallToken,
            relationId: orderInfo.relationId
          },
