...
Page(Object.assign({}, commHandle, Zan.Toast, baseHandle, {
    ...
    wx.request({
      ...
      success: function (res) {
        if (res.data.ret == 0) {
          ...
          if (typeof coupon.customButton == 'string') {
            try {
              var buttonColor = JSON.parse(coupon.customButton);
              that.setData({
                customButton: buttonColor.customButton || "#04bd60",
                donationButton: buttonColor.donationButton || "#FF0000"
              });
            }
            catch (e) {
              that.setData({
                customButton: "#04bd60",
                donationButton: "#FF0000"
              });
            }
          }
  ...
  onShareAppMessage: function () {
    ...
    /*
    *
    * wx.navigateTo({ url: 'sharecoupon?id='+this.data.id+'&recordId='+this.data.recordId+'&openid='+this.data.record.openid, })
    * */
    ...
    return {
      title: 'Ta给你送了一张优惠券，快来领取吧！', // 转发后 所显示的title
      path: '/pages/coupon/sharecoupon?id=' + that.data.id + '&recordId=' + that.data.recordId+"&fromUid=" + (app.globalData.fromuid || '') + "&shopUid=" + (app.globalData.shopuid || ''), // 相对的路径
      imageUrl: imageUrl,
      success: (res) => {    // 成功后要做的事情
        ...
        // console.logcoupon?id={{record.couponId}}&recordId={{record.id}}
      },
      fail: function (res) {
        // 分享失败
        console.log(res)
